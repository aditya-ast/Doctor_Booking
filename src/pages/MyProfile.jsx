...
const MyProfile = () => {
  ...
  const[userData, setUserData] = useState({
    ...
    email: "aditya.jadon8268@gmail.com",
    phone: 9863722538,
    address: {
      line1: "57th Cross Richamod",
  ...
  return (
    <div className='max-w-lg flex flex-col gap-2 text-sm text-left'>
      ...
      <div className='mt-10'>
        {
          isEdit
          ? <button className='border px-8 py-2 border-primary rounded-full hover:bg-primary hover:text-white transition-all' onClick={() => setIsEdit(false)}>Save Information</button>
          : <button className='border px-8 py-2 border-primary rounded-full hover:bg-primary hover:text-white transition-all' onClick={() => setIsEdit(true)}>Edit</button>
        }
      </div>
    </div>
  )
